...
2.流程:
前提条件:采购表中`已获批`购买的一批货物方可入库;

采购经理按单一种类来采购,入库,则该一批次便属于"单装"存储类型单元(批次);

2.1.登记录入:   仓管审查`已取货`的采购审单,审单上已有{申请采购单ID/货物名/供应商/数量/价值金额/本批采购经办/是否已获批/货品分类}信息,
...
所需全部信息:货物名/数量/原存储区域/同意放行与否+提货申请者+目的地+审核部批准意见+出库时间+经手仓管;
...
销售模块:
    ...
    0."单一种类销售单":本次售出商品/客户名/供货商[VO]/定价金额/支付方式/客户是否已付款(0未,1已付定金,2多于定金少于全款,3已全款付完)/客户订购数量/销售负责人(普通雇员或销售经理)/销售时间/现今是否有足够存货(0-无,1-少量,2-半数左右,3-勉强供应,4-完全满足);
    0型付款客户,
